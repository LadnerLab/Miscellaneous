...
def main():
    ...
    p.add_option('-t', '--time', default="6:00:00", help='String to use to set slurm time limit on each job. Default is for 6 hours. [6:00:00]')
    ...
    for i,each in enumerate(args):
        for a in opts.alg.split(","):
            outstr = "%s_%03d" % (a,i)
            cmd="sbatch -J %s -o %s.out -e %s.err -c %d -t %s --mem=%s --wrap='%s --quiet --thread %s %s >%s_mafft-%s.fasta'" % (outstr, outstr, outstr, opts.threads, opts.time, opts.mem, a, opts.threads, each, ".".join(each.split(".")[:-1]), a)
            print (cmd)
